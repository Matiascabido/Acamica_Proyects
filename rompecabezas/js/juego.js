...
var instrucciones = [
  "Seleccionar el Rompecabezas",
  "Visualizar el rompecabezas armado",
  "Colocar su nombre",
  "Iniciar el juego",
  "Observar la pieza de color rojo",
  "Utilizar las flechas del teclado para intercambiar piezas",
  "Repetir la instruccion anterior hasta resolver el rompecabezas"];
// Arreglo para ir guardando los movimientos que se vayan realizando

//Variables Globales
var movimientos = [];

var rompeseleccionado = 0;
...
function mostrarInstrucciones(instrucciones) {
  for(let i = 0; i<instrucciones.length;i++)  {
    ...
    let textnode = document.createTextNode(instrucciones[i]);         // Create a text node
    node.appendChild(textnode);              
    document.getElementById("lista-instrucciones").appendChild(node);                // Append the text to <li>
  }
}
  ...
  if(ingame != 0){
    for (let i = 0; i < grilla.length; i++) {
      for (let j = 0; j < grilla.length; j++) {
        if(grilla[i][j] != girllaVictoria[i][j]){
          bandera = false;
          break;
        }
      }
    ...
    alert('Colocar nombre de jugador y luego presionar jugar');
    bandera = false;
  }
  return bandera;
}
...
function mostrarCartelGanador() {
    ingame = 0;
    alert('FELICITACIONES ' + jugador + ' COMPLETASTE EL ROMPECABEZAS DE '+ romptitle +' CON ' + momimientos + ' MOVIMIENTOS');
}
...
function intercambiarPosicionesGrilla(filaPos1, columnaPos1, filaPos2, columnaPos2) {
  var tempA = grilla[filaPos1][columnaPos1];
  var tempB = grilla[filaPos2][columnaPos2];
  grilla[filaPos1][columnaPos1] = tempB
  grilla[filaPos2][columnaPos2] = tempA;
}
...
function iniciar() {
  ...
  capturarTeclas();
  document.getElementById("pica-pica").style.display = "none";
  document.getElementById("adventure-time").style.display = "none";
...
function jugar(){
  if(rompeSelect != 0){
    if(document.getElementById("gamer").value){
      jugador = document.getElementById("gamer").value;
      if(rompeseleccionado === 1){
        grilla = grilla2;
        girllaVictoria = girllaVictoria2;
      }
      mezclarPiezas(45);
      ingame = 1;
      ...
      alert('Coloque un nombre antes de continuar');
    }
  }else
    alert('Seleccione un rompecabezas antes de continuar');
}

//seleccion de rompecabezas Pickachu
function rmpP(){
  document.getElementById("adventure-time").style.display = "none";
  document.getElementById("pica-pica").style.display = "block";
  rompeSelect = 1;
  rompeseleccionado = 0;
  romptitle ="'PIKACHU'"
}
//seleccion de rompecabezas Adventure Time
function rmpA(){
  document.getElementById("adventure-time").style.display = "block";
  document.getElementById("pica-pica").style.display = "none";
  rompeSelect = 1;
  rompeseleccionado = 1;
  romptitle = "'ADVENTURE TIME'";
}

function ultimoMov(direccion){
  if(direccion == 38) {    
