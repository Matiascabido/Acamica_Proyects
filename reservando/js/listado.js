...
Listado.prototype.buscarRestaurante = function(id) {
    ...
    //     }
    // }
    let restaurante = this.restaurantes.find(res => res.id === id);
  if(restaurante) 
    return restaurante 
  else 
...
Listado.prototype.obtenerCiudades = function() {
    ...
    let ciudades = [];
   // this.restaurantes.forEach(res =>ciudades.push(res.ubicacion));
    ciudades = this.restaurantes.map(res => res.ubicacion);

   // var ciudadesUnicasYOrdenadas = ciudades.filter(function(elem, index, self) { return index === self.indexOf(elem)});

    return this.ordenar(ciudades).sort();
}
...
Listado.prototype.obtenerRubro = function() {
    ...
    // let rubroUnicoYOrdenado = rubro.filter(function(elem, index, self) {return index === self.indexOf(elem)});

    // return r2.sort();

    let Rubro = [];
    Rubro = this.restaurantes.map(res =>  res.rubro );
   // console.log(Rubro);
    return this.ordenar(Rubro).sort();
}
...
Listado.prototype.obtenerHorarios = function() {
    ...
    let Horarios = [];
   // this.restaurantes.forEach(res => Horarios.push(res.horarios));
    Horarios = this.restaurantes.map(res => res.horarios);

    let horario = [];
    Horarios.forEach(horas => {horas.forEach(hora => horario.push(hora))});

  // let horari2 = horario.filter(function(elem, index, self) {return index === self.indexOf(elem)});
 
    return this.ordenar(horario);
}


Listado.prototype.ordenar = function(Aordenar) {
  
  let arreglo = Aordenar.filter(function(elem, index, self) {return index === self.indexOf(elem)});
